...
function handleFile(event) {
    ...
    reader.onload = (e) => {
        ...
        df = df.map((record, index) => {
            const cell = firstSheet[`O${index + 2}`]; // Assuming 'Link de imagen' is in column O
            const url = extractUrl(cell ? cell.f : "");
            return { ...record, 'Link de imagen': url };
        });
...
function extractUrl(formula) {
    if (formula && typeof formula === 'string') {
        const match = formula.match(/"(https:\/\/plataforma\.rrvsac\.com\/api\/event-photos?[^"]+)"/);
        if (match && match[1]) {
            return match[1];
        }
    }
...
function showHyperlink() {
    if (currentIndex < df.length) {
        ...
        const Grupo = record['Empresa'];  
        const Alerta = record['Evento'];
        const Fecha = record['Fecha inicial'];
        const hora = record['Hora inicio'];
        const velocidad = record['Velocidad de conduccion (kph)'];
        const url = record['Link de imagen'];
        currentUrl = url;
...
document.getElementById('downloadButton').addEventListener('click', () => {
    const worksheet = XLSX.utils.json_to_sheet(df.map(record => {
        return {
            ...record,
            'Link de imagen': `=HIPERVINCULO("${record['Link de imagen']}")`
        };
    }));
